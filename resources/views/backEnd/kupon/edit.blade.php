    ...
    <section class="section">
        <div class="section-header shadow-lg">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item active">Edit Coupons</div>
            </div>
        </div>

        <div class="section-body">
            <div class="row">
                <form class="col-12" action="{{ route('coupons.update' , $kupon->id) }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="nama_kupon" class="form-control @error('nama_kupon') is-invalid @enderror"
                                    id="nama_kupon" placeholder="Nama Kupon..." value="{{ $kupon->nama_kupon }}">
                                @error('nama_kupon')
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="jumlah_kupon"
                                    placeholder="Masukan jumlah kupon" name="jumlah_kupon" min="0" value="{{ $kupon->jumlah_kupon }}">
                            </div>

                            <div class="form-group">
                                <label for="nama">Tipe Kupon</label>
                                <select class="custom-select" name="tipe_kupon" id="tipe_kupon">
                                    <option>Pilih tipe kupon</option>
                                    <option value="Voucher Code" {{ ($kupon->tipe_kupon == 'Voucher Code') ? 'selected' : '' ;}}>Voucher Code</option>
                                    <option value="Ticket Code" {{ ($kupon->tipe_kupon == 'Ticket Code') ? 'selected' : '' ;}}>Ticket Code</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group" id="event">
                                ...
                                <select class="custom-select" name="event_id">
                                    ...
                                    <option value="{{ $event->id }}" {{ ($kupon->event_id == $event->id) ? 'selected' : '' ;}}>{{ $event->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group" id="kode">
                                ...
                                <input type="text" class="form-control" id="kode_kupon"
                                    placeholder="Masukan Kode Kupon" name="{{ $kupon->kode_kupon }}">
                            </div>

                            <div class="form-group">
                                <label for="nama">Tipe Potongan</label>
                                <select class="custom-select" name="tipe_potongan" id="tipe_potongan">
                                    <option>Pilih tipe potongan</option>
                                    <option value="Persentase" {{ ($kupon->tipe_potongan == 'Persentase') ? 'selected' : '' ;}}>Persentase</option>
                                    <option value="Harga" {{ ($kupon->tipe_potongan == 'Harga') ? 'selected' : '' ;}}>Harga</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="value"
                                    placeholder="Masukan jumlah potongan" name="value" value="{{ $kupon->value }}">
                            </div>

                            <div class="form-group">
                                <label for="kode_kupon">Tanggal Kadaluarsa</label>
                                <input type="date" class="form-control" id="kadaluarsa"
                                    placeholder="Tanggal Kadaluarsa" name="kadaluarsa" value="{{ $kupon->kadaluarsa }}">
                            </div>
                        </div>
...
<script>
   // console.log($('#tipe_kupon option:selected').val());
   $('#event').hide();
   $('#kode').hide();
   if ($('#tipe_kupon option:selected').val() == 'Voucher Code') {
      $('#kode').show();
   }else if ($('#tipe_kupon option:selected').val() == 'Ticket Code') {
      $('#event').show();
   }
